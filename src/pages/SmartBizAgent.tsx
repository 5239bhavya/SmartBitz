  ...
  return (
    <div className="min-h-screen flex flex-col bg-background relative overflow-hidden">
      ...
      <main className="flex-1 container max-w-7xl py-4 md:py-8 flex gap-4 h-[calc(100vh-64px)]">
        ...
        <Card
          variant="glass"
          className="flex-1 flex flex-col shadow-2xl h-full overflow-hidden border-border/50 backdrop-blur-3xl"
        >
          <CardHeader className="bg-background/40 backdrop-blur-md border-b border-border/50 flex flex-row items-center justify-between py-4 px-6 relative z-10">
            <div className="flex items-center gap-4">
              {/* Desktop/Tablet Back Button */}
              <Button
                variant="ghost"
                size="icon"
                className="hidden md:flex text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded-full"
                onClick={() => navigate("/")}
                title="Go Back"
              >
              ...
              <div>
                ...
                <CardDescription className="text-xs font-medium ml-10 hidden sm:block">
            ...
            <ScrollArea className="h-full p-4">
              <div className="space-y-4 pb-4">
                {messages.map((msg, i) => (
                  ...
                  >
                    <div
                      className={`max-w-[85%] md:max-w-[75%] rounded-3xl px-5 py-3.5 shadow-sm ${msg.role === "user"
                          ? "bg-primary text-primary-foreground rounded-tr-sm"
                          : "bg-muted/40 border border-border/50 text-foreground rounded-tl-sm hover:shadow-md transition-shadow"
                        }`}
                    >
                      ...
                      <p className="whitespace-pre-wrap text-sm md:text-[15px] leading-relaxed">
                        {msg.content}
                      </p>
                    </div>
                  </div>
          ...
          <CardFooter className="p-4 md:p-6 bg-background/50 backdrop-blur-xl border-t border-border/50">
            <form onSubmit={handleSubmit} className="flex relative w-full items-center">
              <Input
                ref={inputRef}
                placeholder="Message SmartBiz AI..."
                value={input}
                onChange={(e) => setInput(e.target.value)}
                disabled={isLoading}
                className="w-full pl-6 pr-24 py-6 rounded-full glass-subtle border-border/50 focus-visible:ring-primary shadow-inner text-base"
                autoFocus
              />
