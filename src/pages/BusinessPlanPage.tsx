...
function ProductMaterialsView({
  ...
  // ✅ Reset selection when a DIFFERENT plan is loaded (items list changes)
  useEffect(() => {
    setSelected(items[0] || "");
  }, [items.join(","), businessType]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* RecipeFlow for selected product — key includes businessType so different plans don't share cache */}
      {selected && (
        <RecipeFlow
          key={`${businessType}::${selected}`}
...
const BusinessPlanPage = () => {
  ...
  // Timestamp that bumps every time a transaction is added — triggers FinancialCharts refetch
  ...
  useEffect(() => {
    ...
    if (storedBusiness && storedProfile) {
      ...
      if (loadedPlan) {
        // ✅ SAVED PLAN: Always use the saved data exactly as-is.
        // NEVER regenerate from selectedProduct — the saved plan has its own product data.
        const parsedPlan = JSON.parse(loadedPlan);
        const fullPlan = { idea: business, ...parsedPlan };
        setPlan(fullPlan);
        // Store so Dashboard loads the correct plan
        sessionStorage.setItem("currentGeneratedPlan", JSON.stringify(fullPlan));
        // Clear any stale product from a different plan
        sessionStorage.removeItem("selectedProduct");
        sessionStorage.removeItem("loadedPlan");
        setIsLoading(false);
      } else {
        // NEW PLAN: Generate fresh from AI using selected business + product
  ...
  const generateBusinessPlan = async (
    ...
    try {
      ...
      setPlan(data);
      // Save generated plan for Dashboard to load
      // ✅ Do NOT remove currentPlanId — we need it to persist across refreshes
      // so that transactions added to this plan remain linked after reload
  ...
  const handleSavePlan = async () => {
    ...
    try {
      ...
      // Award 20 points for saving a business plan
      await supabase.rpc("award_points", {
        p_user_id: user.id,
        p_activity_type: "plan_created",
        p_points: 20,
        p_description: "Created your first business plan"
      });
        ...
        <div className="container max-w-6xl mx-auto space-y-10">
          ...
          <Tabs defaultValue="dashboard" className="space-y-6">
            ...
            <TabsContent value="financials" className="animate-fade-in space-y-8">
              {/* refreshKey bumps every time a transaction is added → triggers live chart update */}
