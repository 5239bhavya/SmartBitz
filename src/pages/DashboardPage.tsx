...
import {
  ...
  ArrowRight, RefreshCw, Trash2, AlertTriangle,
} from "lucide-react";
...
const DashboardPage = () => {
  ...
  const [deleteStep, setDeleteStep] = useState(0); // 0: Idle, 1: First Conf, 2: Final Conf
  const [isDeleting, setIsDeleting] = useState(false);
  const [txTimestamp, setTxTimestamp] = useState(0); // bumps when transaction added → refreshes charts

  // Auto-reset delete button after 5 seconds of inactivity
  useEffect(() => {
    if (deleteStep > 0) {
  ...
  const handleDeletePlan = async () => {
    ...
    if (!planId) {
      toast.error("This plan isn't saved in the database yet. Clear your session to start over.");
      setDeleteStep(0);
      return;
    }
    ...
    try {
      ...
      // 2. Delete all linked financial transactions
      await supabase
        .from("cash_flow")
        .delete()
        .eq("plan_id", planId);

      toast.success("Business plan and all linked data deleted permanently.");

      // 3. Clear session and reset
      sessionStorage.removeItem("selectedBusiness");
      sessionStorage.removeItem("currentGeneratedPlan");
  ...
  return (
    <div className="min-h-screen flex flex-col relative bg-background">
      {/* Enhanced abstract deep gradient backgrounds */}
      <div className="absolute top-0 left-0 right-0 h-[600px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/10 via-background to-background pointer-events-none -z-10" />
      <Header />
      <main className="flex-1 px-4 py-8 md:py-12 relative z-10">
        <div className="container max-w-[1400px] mx-auto space-y-10 relative">

          {/* Decorative blobs */}
          <div className="absolute -top-32 -left-32 w-96 h-96 bg-primary/20 rounded-full blur-[100px] -z-10 pointer-events-none" />
          <div className="absolute top-1/2 -right-32 w-[600px] h-[600px] bg-secondary/10 rounded-full blur-[120px] -z-10 pointer-events-none" />

          {/* ── HERO HEADER ─────────────────────────────────────── */}
          <div className="relative overflow-hidden rounded-[2.5rem] shadow-2xl border border-white/5 bg-black/5 backdrop-blur-3xl animate-slide-up">
            <div className="absolute inset-0 bg-gradient-to-br from-primary/80 via-primary/90 to-secondary/80 opacity-90 mix-blend-multiply" />
            <div className="absolute inset-0 opacity-20 bg-[linear-gradient(rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:3rem_3rem]" />
                ...
                <motion.div className="flex items-center gap-2 flex-wrap" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
                  ...
                  <Button
                    ...
                    className={`rounded-full gap-2 transition-all duration-300 ${
                      deleteStep === 0
                        ? "text-primary-foreground/60 hover:text-white hover:bg-white/10"
                        : deleteStep === 1
                        ? "bg-orange-500/20 text-orange-200 border border-orange-500/50 hover:bg-orange-500/30"
                        : "bg-red-600 text-white shadow-lg shadow-red-500/40 hover:bg-red-700 animate-pulse"
                    }`}
                  ...
                  >
                    {isDeleting ? (
                      ...
                      <>
                        <Trash2 className={`h-4 w-4 ${deleteStep > 0 ? "text-white" : ""}`} />
          ...
          {/* ── TABS ─────────────────────────────────────────────── */}
          <Tabs defaultValue="overview" className="space-y-8 animate-slide-up" style={{ animationDelay: "100ms" }}>
                ...
                <motion.div className="lg:col-span-4 space-y-8" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                  ...
                  <FaceAuthGuard>
                    <CashFlowWidget
                      ...
                      onTransactionAdded={() => setTxTimestamp(Date.now())}
                    />
                  </FaceAuthGuard>
                </motion.div>
              </div>
            ...
            <TabsContent value="financials" className="animate-fade-in space-y-8">
              <FinancialCharts idea={plan.idea} pricing={plan.pricing} planId={planId} refreshKey={txTimestamp} />
