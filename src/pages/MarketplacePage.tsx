...
const MarketplacePage = () => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          allListings = [...allListings, ...govData];
        }
      } catch (apiError) {
  ...
  const filteredListings = listings.filter((listing) => {
    ...
    // 2. Tab Filter (Strict & Case-Insensitive)
    if (activeTab !== "all") {
      const tabValue = activeTab.toLowerCase();
      const listingValue = (listing.listing_type || "").toLowerCase();
      if (listingValue !== tabValue) {
        return false;
      }
    }
          ...
          >
            <div className="overflow-x-auto pb-2 -mx-2 px-2 hide-scrollbar">
              <TabsList className="inline-flex w-auto min-w-full h-auto gap-2 bg-muted/40 backdrop-blur-xl border border-border/50 p-2 rounded-2xl shadow-sm">
                <TabsTrigger value="all" className="whitespace-nowrap px-4 py-2">All Listings</TabsTrigger>
                <TabsTrigger value="sell" className="gap-2 whitespace-nowrap px-4 py-2">
                  <ShoppingBag className="h-4 w-4" />
                  For Sale
                </TabsTrigger>
                <TabsTrigger value="buy" className="gap-2 whitespace-nowrap px-4 py-2">
                  <Package className="h-4 w-4" />
                  Raw Materials
                </TabsTrigger>
                <TabsTrigger value="export" className="gap-2 whitespace-nowrap px-4 py-2">
                        ...
                        <div className="p-5 border-b glass-subtle">
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              ...
                              <Badge
                                variant="outline"
                                className={`text-[9px] uppercase tracking-tighter border-none px-2 py-0.5 rounded ${listing.listing_type === 'sell' ? 'bg-green-500/10 text-green-400' :
                                    listing.listing_type === 'buy' ? 'bg-orange-500/10 text-orange-400' :
                                      'bg-blue-500/10 text-blue-400'
                                  }`}
                              >
                                {listing.listing_type === 'sell' ? 'Manufacturer' :
                                  listing.listing_type === 'buy' ? 'Supplier' : 'Export Partner'}
                              </Badge>
                            </div>
                          </div>
