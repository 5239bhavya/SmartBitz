...
export const KhataDashboard = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [txRes, cashFlowRes, invRes] = await Promise.all([
                supabase
            ...
            // Merge khata transactions with cash_flow entries
            const unified: Transaction[] = (txRes.data || []).map((tx) => ({
                id: tx.id,
            ...
            // Fetch plan names for plan-linked cash_flow entries
            const planLinkedEntries = (cashFlowRes.data || []).filter((tx) => !!tx.plan_id);
            const uniquePlanIds = [...new Set(planLinkedEntries.map((tx) => tx.plan_id))];

            // Build a map of planId -> planName
            ...
            if (uniquePlanIds.length > 0) {
                const { data: plans } = await supabase
                    ...
                    .in('id', uniquePlanIds);
                (plans || []).forEach((p: any) => {
                    planNameMap[p.id] = p.plan_name;
                });
            }

            // Normalize plan-linked cash_flow entries into khata format, labelled with plan name
            if (!cashFlowRes.error && cashFlowRes.data) {
                cashFlowRes.data
                    .filter((tx) => !!tx.plan_id)  // Only plan-linked transactions
                    .forEach((tx) => {
                        const planLabel = planNameMap[tx.plan_id] || 'Business Plan';
                        const catPart = tx.category || '';
                        // If the description already has the plan name prefix (set by CashFlowWidget), use it
                        // Otherwise build one
                        const rawDesc = tx.description || '';
                        const descPart = rawDesc.startsWith(`[${planLabel}]`)
                            ? rawDesc
                            : `[📈 ${planLabel}] ${catPart}${rawDesc ? ' • ' + rawDesc : ''}`.trim();

                        unified.push({
                            id: `cf-${tx.id}`,
                            type: tx.type === 'expense' ? 'Expense' : 'Sale',
                            amount: Number(tx.amount),
                            description: descPart || `[📈 ${planLabel}] Entry`,
            ...
            // Sort newest first
            unified.sort((a, b) => new Date(b.transaction_date).getTime() - new Date(a.transaction_date).getTime());

            setTransactions(unified.slice(0, 30));
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card variant="glass" className="bg-primary/5 border-primary/20 shadow-sm">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                {/* Transactions */}
                <Card variant="glass" className="h-full flex flex-col border-border/50 shadow-sm animate-fade-in">
                    <CardHeader>
