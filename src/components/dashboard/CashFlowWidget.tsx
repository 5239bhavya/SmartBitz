...
import { BusinessIdea, PricingStrategy } from "@/types/business";
import {
  Select,
  SelectContent,
  SelectItem,
...
interface CashFlowWidgetProps {
  planId?: string;
  planName?: string;           // Name of the plan (shown in Khata)
  idea?: BusinessIdea;
  pricing?: PricingStrategy;
  onTransactionAdded?: () => void;  // Callback to refresh FinancialCharts
}

export const CashFlowWidget = ({ planId, planName, idea, pricing, onTransactionAdded }: CashFlowWidgetProps = {}) => {
  const { user } = useAuth();
  ...
  // Helper to extract a number from strings like "₹50,000 - ₹1,00,000" or "₹500"
  ...
  // Derive values from the business plan when available — 0 if no plan data
  const suggestedPrice = parseAmount(pricing?.suggestedPrice) || parseAmount(pricing?.costPrice) || 0;
  const costPrice = parseAmount(pricing?.costPrice) || 0;
  const monthlyRevenue = parseAmount(idea?.expectedRevenue) || 0;
  const estimatedMonthlySales = suggestedPrice > 0 ? Math.round(monthlyRevenue / suggestedPrice) : 0;
  const investment = parseAmount(idea?.investmentRange) || 0;
  const estimatedFixedCosts = investment > 0 ? Math.round(investment * 0.15) : 0;

  // Projection State — auto-filled from plan data (all zero for a brand new plan)
  const [projectionInputs, setProjectionInputs] = useState<CashFlowInputs>({
    ...
    variableCosts: Math.round(costPrice),
    marketingBudget: investment > 0 ? Math.round(investment * 0.05) : 0,
    taxPercent: 10,
  });

  const projectionResult = calculateCashFlow(projectionInputs);

  // Reset projection inputs when plan changes — always re-derive from the specific plan's data
  useEffect(() => {
    const sp = parseAmount(pricing?.suggestedPrice) || parseAmount(pricing?.costPrice) || 0;
    const cp = parseAmount(pricing?.costPrice) || 0;
    const mr = parseAmount(idea?.expectedRevenue) || 0;
    const ms = sp > 0 ? Math.round(mr / sp) : 0;
    const inv = parseAmount(idea?.investmentRange) || 0;
    const fc = inv > 0 ? Math.round(inv * 0.15) : 0;
    const mb = inv > 0 ? Math.round(inv * 0.05) : 0;

    setProjectionInputs({
      price: sp,
      monthlySales: ms,
      fixedCosts: fc,
      variableCosts: Math.round(cp),
      marketingBudget: mb,
      taxPercent: 10,
    });
  }, [planId, idea?.name, pricing?.suggestedPrice, pricing?.costPrice, idea?.expectedRevenue, idea?.investmentRange]);
  ...
  const loadTransactions = async () => {
    setIsLoading(true);
    setDbError(null);
    setTransactions([]); // Clear current view immediately

    // Strict guard: If this is a new plan with no ID yet, it MUST be empty.
    ...
    try {
      // Fetch all user transactions (base query)
      ...
      console.log(`[CashFlowWidget] Loading for planId: "${planId}"`);
      if (error) {
        console.warn("Cash flow load error:", error.message);
      ...
      const filtered = data.filter((tx: any) => {
        if (planId) {
          // Inside a plan: Only show entries tagged with THIS planId
          return tx.plan_id === planId;
        }
        // Outside a plan: Only show entries with NO planId (global)
        return !tx.plan_id;
      });

      console.log(`[CashFlowWidget] Found ${data.length} total, showing ${filtered.length} for this view.`);

      const loaded: Transaction[] = filtered.map((tx: any) => ({
        id: tx.id,
        date: tx.date,
  ...
  const handleAddTransaction = async () => {
    ...
    // Tag with plan_id and plan_name if we're inside a specific plan
    if (planId) {
      payload.plan_id = planId;
    }
    if (planName) {
      // Store plan name in description prefix so Khata can show it
      payload.description = planName
        ? `[${planName}] ${newTransaction.description.trim()}`.trim()
    ...
    try {
      ...
      if (error) {
        ...
        // Award points if they hit their monthly revenue target
        const currentTarget = monthlyRevenue;
        const newTotal = totalIncome + parseFloat(newTransaction.amount);
        if (newTransaction.type === "income" && currentTarget > 0 && newTotal >= currentTarget) {
           const milestoneKey = `milestone-${planId || 'global'}-${user?.id}`;
           if (!localStorage.getItem(milestoneKey) && user?.id) {
              localStorage.setItem(milestoneKey, "true");
              await supabase.rpc("award_points", {
                 p_user_id: user.id,
                 p_activity_type: "revenue_milestone",
                 p_points: 50,
                 p_description: "Hit a revenue growth milestone!"
              });
              toast.success("🎉 +50 Points for hitting your revenue target!");
           }
        }

        // Notify parent to refresh FinancialCharts
        onTransactionAdded?.();
      }
